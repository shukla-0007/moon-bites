...
router.post("/", async (req, res) => {
  ...
  // 1. Sanitize label
  let cleanLabel = (label || "My Meal Plan").trim().replace(/<[^>]*>/g, ""); // Strip HTML tags
  if (cleanLabel.length > 50) {
    cleanLabel = cleanLabel.substring(0, 50);
  }

  // 2. Validate time format (HH:MM 24-hour)
  const timeRegex = /^([0-1]?[0-9]|2[0-3]):[0-5][0-9]$/;
  if (!timeRegex.test(time)) {
    return res.status(400).json({ message: "Invalid time format. Must be HH:MM in 24-hour format." });
  }

  // 3. Validate days array content
  const validDays = ["Sun", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat"];
  const hasInvalidDay = days.some(d => !validDays.includes(d));
  if (hasInvalidDay) {
    return res.status(400).json({ message: "Invalid days specified. Allowed days: Sun, Mon, Tue, Wed, Thu, Fri, Sat." });
  }

  // 4. Validate budget limits (e.g. ₹50 to ₹5000)
